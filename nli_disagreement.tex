...
\begin{abstract}
Natural language inference (NLI) asks whether a hypothesis follows from a premise, contradicts it, or is neither. Standard benchmarks still assign one gold label to each pair, even though people often disagree. ChaosNLI treats that disagreement as part of the evaluation: models are scored against a large set of human judgments, not only against a majority vote.

Earlier encoder models already showed that high majority-label accuracy does not mean a good match to human disagreement. What remained unclear is whether later pretrained NLI models, trained on larger and more diverse NLI collections, close that gap. This report evaluates two such models, DeBERTa-v3 and ModernBERT, as frozen classifiers on ChaosNLI. We compare them with the original 2020 encoder baselines using both majority-label and distributional metrics, and we look at error patterns, confidence, and a simple calibration step.

The two models do not improve the same things. One is stronger on majority-label accuracy; the other is closer to the human label distribution. Both remain overconfident where annotators split. Extra NLI training data does not, on its own, make distribution matching a solved problem.
\end{abstract}

\section{Introduction}

Natural language inference (NLI) asks whether a hypothesis is entailed by a premise, contradicts it, or is neither. Widely used corpora such as SNLI \citep{bowman2015snli} and MNLI \citep{williams2018mnli} treat each pair as having one correct label, usually the majority of a handful of annotators. That setup hides a lot of real disagreement. \citet{pavlick2019inherent} showed that many of these disagreements persist even when more judgments are collected, and that they cannot simply be treated as annotation noise.

ChaosNLI \citep{nie2020chaosnli} made that disagreement measurable. Selected SNLI and MNLI-matched development examples were re-annotated with 100 labels each, so a model can be compared with a human label distribution as well as with a majority vote. \citet{nie2020chaosnli} then evaluated strong 2020 encoders (BERT, RoBERTa, XLNet, BART, and ALBERT) against those distributions. Two results still matter here. Models that did well on ordinary accuracy were not the ones that matched the human distribution, measured by Jensen--Shannon distance (JSD) and Kullback--Leibler (KL) divergence. Accuracy was high when annotators already agreed and close to chance when they did not.

Since then, pretrained NLI models have continued to improve on standard majority-label benchmarks, often by training on larger mixes of NLI data. The research gap is whether that progress also improves the match to human disagreement. Two widely used later models, DeBERTa-v3 and ModernBERT, are a natural test of this question. They were released after the ChaosNLI study, they are trained as 3-way NLI classifiers on multiple datasets, and they were not trained to predict ChaosNLI's 100-annotator distributions. A recent calibration study evaluates a smaller DeBERTa-v3 variant on ChaosNLI \citep{tao2026confidence}, but not the large multi-dataset DeBERTa-v3 model used in practice, and not ModernBERT.

This report is an error and generalization analysis of existing models, not a new training method. We run the two frozen models on ChaosNLI-SNLI and ChaosNLI-MNLI-matched and compare them with the original encoder baselines. The evaluation uses the same majority-label and distributional metrics as ChaosNLI, then looks at where the models fail, whether their confidence tracks human uncertainty, and whether a simple post-hoc calibration step improves the distribution scores without changing predicted labels.

Later NLI training still does not make majority accuracy and distribution matching the same skill. DeBERTa-v3 is the stronger majority-label model on SNLI; ModernBERT is closer to the human distributions on both subsets. Both stay overconfident on high-disagreement items, often pick the same label there, and lose a large share of MNLI entailment cases to neutral.

The rest of the report is organized around three questions:
\begin{itemize}[itemsep=0.4em, topsep=0.4em]
    \item How well do these two models match ChaosNLI human label distributions, relative to the 2020 encoder baselines?
...
Human disagreement has been studied in NLI because of the difficulty of assigning a single label to some sentence pairs. \citet{pavlick2019inherent} showed that disagreements between annotators often persist even when more judgments are collected and argued that they cannot simply be treated as annotation noise. They also found that the uncertainty expressed by NLI models does not necessarily correspond to human uncertainty. Their work highlights the need to consider more than a single aggregate label when evaluating NLI models.

ChaosNLI \citep{nie2020chaosnli} builds on this idea by collecting 100 annotations for selected examples from SNLI, MNLI-matched, and $\alpha$NLI. The resulting label distributions make it possible to assess how closely model predictions reflect the range of human judgments. The ChaosNLI majority label differs from the original dataset label for a substantial share of examples, and the level of human disagreement varies considerably across items.

\citet{nie2020chaosnli} compared several NLI models, including BERT, RoBERTa, XLNet, BART, ALBERT, and DistilBERT, using both accuracy and distributional measures such as JSD and KL divergence. DistilBERT achieved comparatively strong KL scores and weak accuracy. The authors interpreted this as evidence that majority classification and distribution matching are not the same skill. They also found that model accuracy decreased as human disagreement increased. These results show why both distributional matching and majority-label accuracy are relevant when evaluating models under human disagreement.

\subsection{Modeling and calibrating human disagreement}

Later work explored ways to make NLI models better represent human disagreement. \citet{zhou2022distributed} introduced Distributed NLI, where models predict the distribution of human judgments instead of a single label. They showed that methods such as recalibration and distribution estimation can improve the match between model predictions and human opinion distributions. However, the best results remained below the estimated human upper bound.

\citet{meissner2021capturing} changed the training target from a single gold label to a human label distribution. Their results indicated that training on these distributions can reduce JSD on ChaosNLI. \citet{wang2022calibrated} took a different approach and focused on calibration. They found that label smoothing and temperature scaling can help model probabilities capture the distribution of human judgments.

\citet{baan2022stop} studied calibration when human annotators disagree. They argued that ordinary calibration metrics based on a single majority label are misleading when the ground truth itself is a distribution. They proposed measures that take the full human judgment distribution into account.

\citet{lee2023dissenting} evaluated instruction-tuned language models on ChaosNLI and found that these models struggled to match human disagreement distributions. For example, GPT-3 did not beat RoBERTa-large on JSD. Their study focuses on generative models and does not evaluate the two encoder models we use.

Overall, these studies show that majority-label prediction and human-distribution matching are different goals. They also suggest that calibrating model probabilities can improve distributional matching. What they do not settle is whether later encoder NLI models, still trained with a single gold label, already close the ChaosNLI gap without a change of training target.

\subsection{Recent NLI models and the remaining gap}

Since the original ChaosNLI study, newer pretrained NLI models have been developed, and some have also been evaluated under human disagreement. \citet{tao2026confidence}, for example, evaluated DeBERTa-v3-base and RoBERTa-large on ChaosNLI and compared several post-hoc calibration methods. Their study provides a recent reference for evaluating model confidence when human annotators disagree. However, DeBERTa-v3-base is not the large multi-dataset DeBERTa-v3 model that is widely used for NLI, and their study does not include ModernBERT.

Two points were therefore still unclear. First, it was not known how the widely used large DeBERTa-v3 NLI model of \citet{laurer2024less} and the multi-task ModernBERT NLI model of \citet{sileo2024tasksource} would score on the original ChaosNLI distributional metrics. Neither model card reports those numbers, and neither model was trained to predict the 100-annotator ChaosNLI target. Second, it was not known whether any gain in majority-label accuracy for these models would come with a better match to human disagreement, or whether the split documented by \citet{nie2020chaosnli} would remain.

That is the objective of this report. We evaluate DeBERTa-v3 and ModernBERT as frozen pretrained models under the ChaosNLI protocol, and we analyze how they fail: majority accuracy versus distribution match, confidence versus human disagreement, and SNLI versus MNLI.

\section{Dataset}

We use two subsets of ChaosNLI \citep{nie2020chaosnli}: ChaosNLI-SNLI and ChaosNLI-MNLI-matched. ChaosNLI-SNLI contains 1,514 examples and ChaosNLI-MNLI-matched contains 1,599. Each example has 100 crowd labels, a majority label, a normalized label distribution over entailment, neutral, and contradiction, and the original SNLI or MNLI label. We do not use ChaosNLI-$\alpha$NLI, which is an abductive two-choice task rather than 3-way NLI.

Human entropy is computed from the 100-label distribution, in bits. Mean entropy is 0.80 on SNLI and 1.07 on MNLI. A quarter of SNLI items have entropy above 1 bit; on MNLI that share is 64\%. The original dataset label disagrees with the 100-annotator majority on 25\% of SNLI items and 32\% of MNLI items. ChaosNLI-SNLI majority labels are skewed toward neutral (813 N, 421 E, 280 C). ChaosNLI-MNLI majority labels are skewed toward entailment (741 E, 583 N, 275 C). That shift matters later: both models lose a lot of entailment recall when they move from SNLI to MNLI.

\section{Methods}

We evaluate DeBERTa-v3 and ModernBERT on ChaosNLI using classification, distributional measures, and qualitative analysis. Their predictions are compared with the human label distributions and the original ChaosNLI baselines. We also analyze model confidence and agreement between the two models. Finally, temperature scaling is used to test whether adjusting model confidence improves distributional matching without changing the predicted labels.

\subsection{Models}

The two pretrained NLI models are:

\begin{itemize}[itemsep=0.4em, topsep=0.4em]
\item \textbf{DeBERTa-v3} \citep{he2023debertav3,laurer2024less}. Fine-tuned on MNLI, FEVER-NLI, ANLI, LingNLI, and WANLI. SNLI was left out of training because of known label issues. Released model: \path{MoritzLaurer/DeBERTa-v3-large-mnli-fever-anli-ling-wanli}.
\item \textbf{ModernBERT} \citep{warner2024modernbert,sileo2024tasksource}. Multi-task fine-tuned on a large set of NLI-format datasets, including MNLI, ANLI, WANLI, LingNLI, SICK, and several logic and document NLI tasks. Released model: \path{tasksource/ModernBERT-large-nli}.
\end{itemize}

The two models differ in both architecture and the NLI datasets used for fine-tuning. Both include WANLI (Worker-AI Collaboration for NLI), which was created through human-AI collaboration to construct challenging NLI examples \citep{liu2022wanli}. Neither model was trained to predict the 100-annotator label distributions used by ChaosNLI. Both are 3-way NLI classifiers, which is why we omit $\alpha$NLI.

\subsection{Data processing}

We split each ChaosNLI subset into tertiles of human entropy and call them low, medium, and high disagreement. On SNLI the cuts fall at 0.68 and 0.97 bits; on MNLI they fall at 0.99 and 1.19 bits. The high-entropy SNLI bin is therefore an easier cut than the high-entropy MNLI bin.
...
We begin by comparing the two models with the original ChaosNLI baselines. We then look more closely at error tendencies, including confusion against the 100-annotator majority and cases where the original dataset label disagrees with that majority. The remaining analyses consider entropy bins, agreement between the two models, temperature scaling, and qualitative examples.
...
On SNLI, DeBERTa-v3 has the highest new accuracy (0.793) and the worst JSD and KL. Its old SNLI accuracy (0.709) is below RoBERTa-large (0.749). That split fits the model card: SNLI was not used in training. The model lines up better with the 100-annotator majority than with the original five-annotator labels, and still produces very peaked distributions (mean confidence 0.947, mean model entropy 0.21 bits, against human entropy 0.80).
...
Higher majority accuracy and better distribution match do not arrive together on these two models.
...
On high-disagreement items, all three models are around 60\% on SNLI and 57\% on MNLI. The low-entropy SNLI bin is nearly solved (about 95\%). That is the same shape \citet{nie2020chaosnli} reported. The newer encoders do not move the high-entropy slice.
...
The 100-annotator majority is neutral. Both models keep the older MNLI label. This is the MNLI mismatch pattern: the models were trained on MNLI and still often follow that convention when 100 annotators vote the other way.
...
\centering
\small
\caption{Where each model is strong or weak on ChaosNLI.}
\label{tab:strengths}
...
Both models are 3-way classifiers trained with cross-entropy on one gold label. That loss is minimized by putting almost all probability on one class. ChaosNLI scores the full softmax against a 100-person distribution that is often spread across two labels. If humans put 20\% on contradiction and the model puts 0.1\%, KL becomes large even when the model's top label is the majority. DeBERTa-v3 can therefore be the strongest majority classifier in the SNLI table and the weakest distribution model at the same time. \citet{nie2020chaosnli} already saw a version of this with DistilBERT. The same split is more extreme on the DeBERTa-v3 model of \citet{laurer2024less}.
...
\bibitem[Baan et al.(2022)]{baan2022stop}
...
\bibitem[Laurer et al.(2024)]{laurer2024less}
...
\bibitem[Lee et al.(2023)]{lee2023dissenting}
...
\bibitem[Liu et al.(2022)]{liu2022wanli}
...
\bibitem[Meissner et al.(2021)]{meissner2021capturing}
...
\bibitem[Nie et al.(2020)]{nie2020chaosnli}
...
\bibitem[Sileo(2024)]{sileo2024tasksource}
Damien Sileo.
...
\bibitem[Tao et al.(2026)]{tao2026confidence}
...
\bibitem[Wang et al.(2022)]{wang2022calibrated}
...
\bibitem[Zhou et al.(2022)]{zhou2022distributed}
